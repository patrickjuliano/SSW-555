...
describe('Test activity functionality', () => {
    ...
    before('Connect to database', async () => {
        ...
        message2 = await activityData.createMessageFromTask(task._id, user._id, content2);
        message3 = await activityData.createMessageFromSubtask(task._id, subtask._id, user._id, content3);
    });
    ...
    describe('Test getting message', () => {
        it('Get project message', async () => {
            ...
            chai.expect(message).to.have.property('content', content1);
        ...
        it('Get task message', async () => {
            ...
            chai.expect(message).to.have.property('content', content2);
        ...
        it('Get subtask message', async () => {
            ...
            chai.expect(message).to.have.property('content', content3);
    ...
    describe('Test getting all messages', () => {
        it('Get all messages', async () => {
            ...
            chai.expect(messages[messages.length - 3]).to.have.property('content', content1);
            ...
            chai.expect(messages[messages.length - 2]).to.have.property('content', content2);
            ...
            chai.expect(messages[messages.length - 1]).to.have.property('content', content3);
    ...
    describe('Test creating message', () => {
        it('Create project message', async () => {
            ...
            chai.expect(message).to.have.property('content', content);
            chai.expect(message).to.have.property('level', 0);
        });

        it('Create task message', async () => {
            const content = 'This is a new task message';
            const message = await activityData.createMessageFromTask(task._id, user._id, content);
            chai.expect(message).to.have.property('_id');
            chai.expect(message).to.have.property('content', content);
            chai.expect(message).to.have.property('level', 1);
        });
        ...
        it('Create subtask message', async () => {
            ...
            const message = await activityData.createMessageFromSubtask(task._id, subtask._id, user._id, content);
            chai.expect(message).to.have.property('_id');
            chai.expect(message).to.have.property('content', content);
            chai.expect(message).to.have.property('level', 2);
        });
    ...
    describe('Test editing message', () => {
        it('Edit project message', async () => {
            ...
            const message = await activityData.editMessage(message1._id, content);
            chai.expect(message).to.have.property('content', content);
        });

        it('Edit task message', async () => {
            const content = 'This is an edited task message';
            const message = await activityData.editMessage(message2._id, content);
            chai.expect(message).to.have.property('content', content);
        });

        it('Edit subtask message', async () => {
            const content = 'This is an edited subtask message';
            const message = await activityData.editMessage(message3._id, content);
            chai.expect(message).to.have.property('content', content);
        });
    });
